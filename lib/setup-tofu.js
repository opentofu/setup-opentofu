  ...
  try {
    ...
    if (versionFile) {
      try {
        ...
        const resolvedVersionFile = resolve(workspace, versionFile);
        if (resolvedVersionFile !== workspace && !resolvedVersionFile.startsWith(workspace + sep)) {
          throw new Error(`tofu_version_file '${versionFile}' resolves outside of the workspace directory`);
        }
        debug(`Reading OpenTofu version from file: ${resolvedVersionFile}`);
        const fileVersion = await fs.readFile(resolvedVersionFile, 'utf8');
