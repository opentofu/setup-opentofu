...
describe('setup-tofu', () => {
  ...
  describe('checksum validation', () => {
    ...
    it('should download and validate checksum when checksums are provided', async () => {
      ...
      // Mock createReadStream to simulate a readable stream
      const mockStream = {
      ...
      // Mock the fs module before importing setup-tofu.js
      ...
      // Mock the crypto module before importing setup-tofu.js
      jest.unstable_mockModule('crypto', () => ({
        createHash: jest.fn(() => ({
          update: jest.fn(() => ({
            digest: jest.fn(() => mockChecksum)
          })),
          digest: jest.fn(() => mockChecksum)
        }))
      }));

      // Mock the stream/promises module before importing setup-tofu.js
      jest.unstable_mockModule('stream/promises', () => ({
        pipeline: jest.fn(() => Promise.resolve())
      }));

      // Re-import the module with the new mocks
      ...
      // Mock the fs.promises.readFile to return a mock file content
      const fs = await import('fs');
      fs.promises.readFile.mockResolvedValue('mock file content');

      // Mock the fs.createReadStream to return a mock stream
      fs.createReadStream.mockReturnValue(mockStream);

      // Mock the pipeline function to resolve successfully
