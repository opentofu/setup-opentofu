...
async function installWrapper (pathToCLI) {
  ...
  // Rename opentofu(.exe) to opentofu-bin(.exe)
  try {
    source = [pathToCLI, `opentofu${exeSuffix}`].join(path.sep);
    target = [pathToCLI, `opentofu-bin${exeSuffix}`].join(path.sep);
  ...
  // Install our wrapper as opentofu
  try {
    source = path.resolve([__dirname, '..', 'wrapper', 'dist', 'index.js'].join(path.sep));
    target = [pathToCLI, 'opentofu'].join(path.sep);
  ...
  core.exportVariable('OPENTOFU_CLI_PATH', pathToCLI);
}

// Add credentials to CLI Configuration File
// https://www.opentofu.io/docs/commands/cli-config.html
...
credentials "${credentialsHostname}" {
  ...
  let credsFile = osPlat === 'win32'
    ? `${process.env.APPDATA}/opentofu.rc`
    : `${process.env.HOME}/.opentofurc`;
...
async function run () {
  try {
    ...
    const version = core.getInput('opentofu_version');
    ...
    const wrapper = core.getInput('opentofu_wrapper') === 'true';
    ...
    const release = await releases.getRelease('opentofu', version, 'GitHub Action: Setup Terraform');
